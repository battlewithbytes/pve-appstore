...
func (s *Server) handleConfigApply(w http.ResponseWriter, r *http.Request) {
	...
	for _, stack := range req.Stacks {
		for _, app := range stack.Apps {
			if _, ok := s.catalog.Get(app.AppID); !ok {
	...
	for _, stack := range req.Stacks {
		job, err := s.engine.StartStack(stack)
		if err != nil {
...
func (s *Server) handleConfigApplyPreview(w http.ResponseWriter, r *http.Request) {
	...
	for i, r := range parsed.Recipes {
		...
		if _, ok := s.catalog.Get(r.AppID); !ok {
...
func (s *Server) handleEditInstall(w http.ResponseWriter, r *http.Request) {
	...
	job, err := s.engine.EditInstall(installID, req)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (s *Server) handleReconfigure(w http.ResponseWriter, r *http.Request) {
	...
	inst, err := s.engine.ReconfigureInstall(installID, req)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
